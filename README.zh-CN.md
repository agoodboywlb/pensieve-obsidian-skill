...
```
┌─────────┐  ┌─────────┐  ┌─────────┐
│ Agent A  │  │ Agent B  │  │ Agent C  │   每个 agent 记忆独立隔离
└────┬─────┘  └────┬─────┘  └────┬─────┘
     │写入         │读写         │读取
     ▼             ▼             ▼
┌──────────────────────────────────────┐
│       Obsidian Vault（本地）          │   共享、持久、结构化
│  索引 -> 摘要 -> 详情                 │
└──────────────────────────────────────┘
```
...
- **可定制模板**：编辑 `templates/*.md` 适配你的 vault 风格
...
编辑 `templates/` 中的文件以调整笔记结构、frontmatter 字段或标签规则。

| 模板 | 用途 |
|------|------|
| `templates/Index.md` | 项目索引骨架 |
| `templates/L1-Summary.md` | 主题摘要骨架 |
| `templates/L2-Detail.md` | 详情笔记（含 frontmatter 和章节） |

占位符使用 `{{variable_name}}` 语法。月份分组和追加逻辑由脚本处理，不在模板中。

## 作为 Claude Code Skill 使用

将此目录复制到你的 Claude Code skills 路径下，通过关键词触发：`obsidian`、`knowledge card`、`archive note`。
